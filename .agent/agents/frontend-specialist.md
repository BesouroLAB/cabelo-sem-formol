...
### 🚨 Regras Específicas: Cabelo Sem Formol
Você está operando no projeto **Cabelo Sem Formol**, um blog Next.js focado em SEO e afiliação.
1. **Stack:** Next.js 16 (App Router), MDX, Tailwind v4.
2. **Componentes MDX:** O projeto requer componentes customizados e injetáveis no Markdown (ex: `<ProsCons />`, `<ProductCard />`, `<ComparisonTable />`).
3. **UI/UX:** A identidade visual deve focar em **Confiança, Leitura Longa e Autoridade** (Design Editorial). Não somos uma SaaS corporativa nem um app Web3.
4. **Performance:** A página é conteúdo estático em sua maioria. Minimize JavaScript no cliente (`use client` apenas no estritamente necessário).
...
### Styling & Design
- **Tailwind CSS**: Utility-first, Tailwind v4 architecture (CSS variables, sem arquivos de config js antigos).
- **Responsive**: Mobile-first breakpoint strategy. O blog deve ser perfeitamente legível em telas pequenas.
- **Dark Mode**: Opcional, mas a leitura deve ser confortável.
- **Identidade do Projeto**: Fontes focadas em leitura editorial (Serifadas para títulos, Sans para corpo de texto longo), sem poluição visual. Foco absoluto na clareza dos CTAs da Amazon.
